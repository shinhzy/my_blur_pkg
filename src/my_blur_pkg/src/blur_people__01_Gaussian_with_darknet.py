...
def bounding_boxes_callback(msg):
    ...
    for box in msg.bounding_boxes:
        if box.Class == "person": # and cv_image is not None:
            ...
            roi = cv_image[box.ymin:box.ymax, box.xmin:box.xmax]
            # blur
            blur = cv2.GaussianBlur(roi, (99, 99), 30)
            # Change Original image to Blured image
            cv_image[box.ymin:box.ymax, box.xmin:box.xmax] = blur

    image_pub.publish(bridge.cv2_to_imgmsg(cv_image, "bgr8"))
    cv2.imshow("Blurred Image", cv_image)
    cv2.waitKey(1)
